...
function init() {  // called once when loading HTML file
  ...
  function getBox(x, y) {
    ...
    colFlag = ((x-135)/75 + (y-135)/75)%2;  // Checkerboard
    if (colFlag == 1 ){
      color = [0, 0, 200];
    }
    else {
...
function senseColor() {
    ...
    function sensorRay(bodies, rayLength) {
        bodies = bodies.filter(body => body.id != parent.body.id);
...
function robotMove(robot) {
 ...
 // The following rules are implemented:
	...
	driveConstant = 0.00025*1.5; //Adjust speed to slower (a bit)
	turnConstant = 0.0025*2/3;    
    ...
    if(objMid == "Red") {
        driveVector(robot, force=-8*driveConstant, angle=-80*turnConstant);     //Ignore Red
    }
	

    if(objLeft == "Robot" || objRight == "Robot"){
        console.log("Robot Detected");
        driveVector(robot, force=-8*driveConstant, angle=-100*turnConstant);                             // ATTEMPT: AVOID OTHER ROBOTS                  
        }
	...
	if(sensesWall){
		switch(objMid){
			case "Empty":
                driveVector(robot, force=0, angle=50*turnConstant);
                break;
			case "Robot":
                driveVector(robot, force=0, angle=-2*turnConstant);
                break;
			case "Blue":
				driveVector(robot, force=0, angle=-40*turnConstant); //driveVector(robot, force=0, angle=-20*turnConstant); 
				break;
			case "Red":
				driveVector(robot, force=0, angle=50*turnConstant); 
				break;
      ...
      switch(objMid){
        case "Empty":
          driveVector(robot, force=driveConstant, angle=2*turnConstant);
          break;
        case "Robot":
          driveVector(robot, force=-8*driveConstant, angle=-80*turnConstant);
          break;
        case "Blue":
          if(objLeft == "Red") {
            driveVector(robot, force=driveConstant, angle=20*turnConstant*5); 
          } else {
            driveVector(robot, force=driveConstant, angle=turnConstant);    
          } //Changed here
          break;
        case "Red":
          driveVector(robot, force=-8*driveConstant, angle=-80*turnConstant); //Ignores Red now
          break;
        default:
          console.log("Error")
      }
    ...
    if(objLeft == "Red") {
      driveVector(robot, force=driveConstant, angle=10*turnConstant);
    } else if (objLeft == "Robot" || objRight == "Robot") {
        driveVector(robot, force=-8*driveConstant, angle=-80*turnConstant);
    } else {
        driveVector(robot, force=driveConstant, angle=1*turnConstant);  
    }
  }
...
function classifyRGB(rgb) {
    ...
    if (r<25&&g<25&&b<25){
        ...
        return("Blue");
    } else if (r>200&&g>200&&b>200) {  // Sure that this one works as we think it does?
        return ("Robot");
...
function updateStatistics() {
  ...
  function calcPercentageTresholdFromCorner(boxesPos,treshold){
      ...
      boxesManhattanDisFromCorner = boxesPos.map(boxPos =>
           Math.min(Math.min(boxPos.x - wallThickness,(width - wallThickness) - boxPos.x) +
           Math.min(boxPos.y - wallThickness,(height - wallThickness) - boxPos.y)));

      boxesWithinTreshFromCorner = boxesManhattanDisFromCorner.filter(boxDist => boxDist < treshold)
