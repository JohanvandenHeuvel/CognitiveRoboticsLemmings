...
function init() {  // called once when loading HTML file
  ...
  function getBox(x, y) {
    ...
    colFlag = ((x-100)/60 + (y-100)/50)%2;  // Checkerboard
    if (colFlag == 1 ){
      color = [0, 0, 200];
    }
    else {
...
function robotMove(robot) {
 ...
 // The following rules are implemented:
    ...
    if(objLeft == "Robot" || objRight == "Robot"){
        ...
        driveVector(robot, force=driveConstant, angle=20*turnConstant);                             // ATTEMPT: AVOID OTHER ROBOTS                  
        }
	...
	if(sensesWall){
      ...
      switch(objMid){
        ...
        case "Blue":
          if(objLeft == "Red") {
            driveVector(robot, force=driveConstant, angle=20*turnConstant); //instead of 4, now 20
          } else {
            driveVector(robot, force=driveConstant, angle=turnConstant);
          } //Changed here
          break;
        case "Red":
          if (objLeft == "Red" || objRight == "Red") {
            driveVector(robot, force=-2*turnConstant, angle=-80*turnConstant); //Leave red block at red blocks
          } else {
                driveVector(robot, force=driveConstant, angle=turnConstant); //added
          }
          break;
        default:
          console.log("Error")
...
function updateStatistics() {
  ...
  function calcPercentageTresholdFromCorner(boxesPos,treshold){
    ...
    console.log(calcPercentageTresholdFromCorner(blueBoxesPos,simInfo.tresHoldInCorner))
